...
const exGrammarMap = {
  ...
  'aanwijzende-vnw': s => s.gtopic === 'aanwijzende-vnw',
  ...
  'klok-tijd': s => s.gtopic === 'klok-tijd',
...
// Skip a sentence you don't want to do: move on with no penalty — doesn't
// touch stats or SRS scheduling, unlike an actual wrong answer.
...
const grammarRulesData = [
  {
    ...
    pattern: 'Pos. 1 (Onderwerp) → Pos. 2 (Werkwoord) → Middenveld (T·I·M·D·P) → Eindpositie',
    nlExpl: 'De GOUDEN REGEL: de positie van het VERVOEGDE werkwoord bepaalt alles. In een hoofdzin staat dat werkwoord ALTIJD op positie 2 (de V2-regel). Onderwerp en werkwoord blijven "getrouwd" naast elkaar, ook na inversie. Alle overige werkwoorden (infinitief, deelwoord, scheidbaar deel) gaan naar het einde. Middenveld-volgorde: Tijd → Indirect object → Manier → Direct object → Plaats (T·I·M·D·P) — Tijd staat altijd vooraan.',
    enExpl: 'The GOLDEN RULE: the position of the FINITE verb determines everything. In a hoofdzin it always sits in position 2 (the V2 rule). Subject and verb stay "married" next to each other, even after inversion. Every other verb (infinitive, participle, separable prefix) goes to the end. Middle-zone order: Time → Indirect object → Manner → Direct object → Place (T·I·M·D·P) — Time always comes first.',
    tip: 'Positie 2 is ALTIJD voor het vervoegde werkwoord. Middenveld: T·I·M·D·P — Tijd staat altijd vooraan en verschuift nooit!',
    conjunctions: [],
    slots: [
      { pos: '1', color: 'pos1', label: 'Onderwerp', en: 'Subject', desc: 'Wie of wat doet de actie?', examples: 'Ik · Jan · De kinderen · Het boek' },
      { pos: '2', color: 'pos2', label: 'Werkwoord', en: 'Conjugated verb', desc: 'Vervoegd naar persoon + tijd. Nooit verplaatsen! "Getrouwd" met het onderwerp.', examples: 'ga · werkt · eten · heeft' },
      { pos: 'M', color: 'posm', label: 'Middenveld  (T → I → M → D → P)', en: 'Middle field  (Time → Indirect object → Manner → Direct object → Place)', desc: 'Vaste volgorde: Tijd (wanneer?) → Indirect object zonder voorzetsel (aan wie?) → Manier (hoe?) → Direct object (wat?) → Plaats (waar?). Tijd staat altijd vooraan.', examples: 'morgen (T) · haar (I) · snel (M) · een boek (D) · naar school (P)' },
      { pos: 'E', color: 'pose', label: 'Eindpositie', en: 'End field', desc: 'Scheidbaar werkwoorddeel, infinitief, of deelwoord. Modaal + infinitief: modaal blijft op pos. 2, infinitief naar het einde.', examples: 'op (scheidbaar) · kopen (inf.) · gekocht (deelwoord)' },
    ],
    wordOrder: [
      { slot: 'Tijd (T)', q: 'Wanneer? — altijd eerst', ex: 'morgen, gisteren, elke dag, om 9 uur' },
      { slot: 'Indirect object (I)', q: 'Aan wie? (zonder voorzetsel = vast; met "aan/voor" = flexibel)', ex: 'haar, hem, de kinderen' },
      { slot: 'Manier (M)', q: 'Hoe?', ex: 'snel, graag, samen, goed' },
      { slot: 'Direct object (D)', q: 'Wat?', ex: 'een boek, de brief, dit' },
      { slot: 'Plaats (P)', q: 'Waar? — kan wisselen met direct object voor nadruk', ex: 'thuis, in Amsterdam, naar school' },
    ],
  },
  {
    stype: 'Hoofdzin + inversie', en: 'Inversion',
    pattern: 'Voorveld (≠ onderwerp) → Werkwoord (pos. 2) → Onderwerp (pos. 3) → Middenveld (T·I·M·D·P) → Eindpositie',
    nlExpl: 'Inversie is geen aparte regel — het is gewoon de V2-regel in actie zodra iets anders dan het onderwerp op positie 1 staat. Het werkwoord blijft op positie 2, maar het onderwerp schuift naar positie 3. Typische triggers: een tijdsbepaling (morgen, gisteren), een plaatsbepaling (hier, in Amsterdam), "daarom"/"dus", een benadrukt object, of een hele bijzin die vooropstaat.',
    enExpl: 'Inversion is not a separate rule — it is simply the V2 rule in action when the subject is not first. The verb stays in position 2, but the subject moves to position 3. Common triggers: a time expression, a place expression, "daarom"/"dus", an emphasised object, or a whole bijzin placed up front.',
    tip: 'Beginwoord ≠ onderwerp? → werkwoord blijft op pos. 2, dan pas het onderwerp op pos. 3! "Daarom" en "dus" vooraan triggeren altijd inversie.',
    conjunctions: [],
    slots: [
      { pos: '1', color: 'pos1', label: 'Voorveld (NIET het onderwerp)', en: 'Front field (NOT the subject)', desc: 'Tijdsbepaling, plaatsbepaling, bijwoord ("daarom"), bijzin, of object dat je wil benadrukken.', examples: 'Morgen · Daar · Daarom · Op maandag · Snel' },
      ...
      { pos: 'M', color: 'posm', label: 'Middenveld  (T → I → M → D → P)', en: 'Middle field  (Time → Indirect object → Manner → Direct object → Place)', desc: 'Rest van de zin in T-I-M-D-P volgorde.', examples: 'snel · haar · naar huis · een koffie' },
      { pos: 'E', color: 'pose', label: 'Eindpositie', en: 'End field', desc: 'Infinitief, deelwoord of scheidbaar deel.', examples: 'fietsen · gegeten · op' },
    ],
    wordOrder: [
      { slot: 'Voorveld', q: 'Wat benadrukt je?', ex: 'Morgen, Daar, Daarom, Elke dag, Gisteren' },
      { slot: 'Werkwoord', q: 'Positie 2 (altijd!)', ex: 'gaat, werkt, is, heeft' },
      { slot: 'Onderwerp', q: 'Positie 3 — nooit pos. 2', ex: 'ik, hij, de leraar' },
    ],
  },
  {
    stype: 'Hoofdzin + bijzin', en: 'Main clause + subordinate clause',
    pattern: '[Hoofdzin] + Voegwoord + Onderwerp + Middenveld (T·I·M·D·P) + ALLE werkwoorden',
    nlExpl: 'De bijzin begint met een onderschikkend voegwoord. In de bijzin staat het WERKWOORD (ook het vervoegde deel) aan het EINDE — niet alleen de infinitief zoals in een hoofdzin. Bij modaal + infinitief komt het modale werkwoord vóór de infinitief: "omdat ik wil kijken" ✓, NIET "omdat ik kijken wil" ✗. Bij de voltooide tijd zijn beide volgordes toegestaan in gesproken taal: "omdat je het hebt gezien" of "omdat je het gezien hebt". Scheidbare werkwoorden blijven in de bijzin één woord: "omdat ik vroeg opsta" (niet gesplitst).',
    enExpl: 'The subordinate clause starts with a subordinating conjunction. ALL verbs (including the finite one) move to the END — not just the infinitive as in a hoofdzin. With modal + infinitive, the modal comes BEFORE the infinitive: "omdat ik wil kijken" ✓, NOT "omdat ik kijken wil" ✗. In the perfect tense both orders are accepted in spoken Dutch. Separable verbs stay together as one word in a bijzin.',
    tip: 'Bijzin = alles normaal BEHALVE: alle werkwoorden gaan naar het einde! Modaal ALTIJD vóór infinitief ("wil kijken", nooit "kijken wil"). Scheidbare werkwoorden blijven bij elkaar.',
    ...
    slots: [
      ...
      { pos: 'VW', color: 'posvw', label: 'Voegwoord', en: 'Conjunction', desc: 'Leidt de bijzin in. Signaal: alle werkwoorden gaan naar einde!', examples: 'omdat · dat · als · hoewel · terwijl' },
      ...
      { pos: 'M', color: 'posm', label: 'Middenveld  (T → I → M → D → P)', en: 'Middle field', desc: 'Tijdsbepaling, indirect object, manier, direct object, plaats — in T-I-M-D-P volgorde.', examples: 'morgen (T) · haar (I) · snel (M) · een boek (D) · naar school (P)' },
      { pos: 'E', color: 'pose', label: 'Werkwoord(en) aan het einde', en: 'Verb(s) at the end', desc: 'Modaal vóór infinitief. Bij vtt: beide volgordes mogelijk. Scheidbare werkwoorden blijven één woord.', examples: 'werkt · heeft gewerkt / gewerkt heeft · wil werken · opstaat' },
    ],
    wordOrder: [
      { slot: 'Voegwoorden', q: 'Onderschikkend', ex: 'omdat, dat, als, terwijl, hoewel, nadat, zodat, toen' },
      { slot: 'Einde bijzin', q: 'Werkwoordvolgorde', ex: 'enkelvoud: "werkt" | vtt: "heeft gewerkt" of "gewerkt heeft" | modaal: "wil werken" (nooit "werken wil")' },
    ],
  },
  {
    stype: 'Bijzin + hoofdzin', en: 'Subordinate clause first → inversion',
    pattern: '[Voegwoord + Onderwerp + Middenveld + ALLE werkwoorden], → Werkwoord + Onderwerp + Rest',
    nlExpl: 'Als de bijzin vóór de hoofdzin staat, telt de hele bijzin als positie 1 van de hele zin. Daarna komt direct het werkwoord van de hoofdzin op positie 2 (verplichte inversie), en pas daarna het onderwerp. Veelgemaakte fout: "Omdat ik moe ben, ik ga slapen" ✗ — het werkwoord staat hier niet op positie 2. Correct: "Omdat ik moe ben, ga ik slapen" ✓.',
    enExpl: 'When the subordinate clause leads, the whole bijzin occupies position 1. The main clause verb must immediately follow, in position 2 (mandatory inversion), before the subject. Common mistake: "Omdat ik moe ben, ik ga slapen" ✗. Correct: "Omdat ik moe ben, ga ik slapen" ✓.',
    tip: 'De bijzin = positie 1 als blok → daarna werkwoord (verplicht!) → dan pas onderwerp. Vergeet nooit de komma tussen bijzin en hoofdzin.',
    conjunctions: ['omdat', 'als', 'terwijl', 'hoewel', 'toen', 'nadat', 'zodra'],
    slots: [
      { pos: '1', color: 'posvw', label: 'Bijzin (= heel blok op pos. 1)', en: 'Subordinate clause (= position 1 block)', desc: 'De hele bijzin functioneert als positie 1. Interne volgorde: Vgw. + S + TIMDP + Werkwoord(einde).', examples: 'Omdat hij ziek is · Als het regent · Toen ik aankwam' },
      ...
      { pos: 'M', color: 'posm', label: 'Rest hoofdzin', en: 'Rest of main clause', desc: 'Normaal T-I-M-D-P volgorde.', examples: 'thuis · snel · een koffie' },
    ],
    wordOrder: [],
  },
  {
...
const GRAMMAR_CLOZE = {
  ...
  'aanwijzende-vnw':   { words: ['dit','deze','dat','die'] },
  ...
  'klok-tijd':         { words: ['uur','over','voor','half','kwart'] },
